...
const Designers = () => {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5000/api/designers')
      .then((response) => {
        setDesigners(response.data);
        setFilteredDesigners(response.data);
        setLoading(false);
      })
      .catch((err) => {
  ...
  if (noFilters) {
    ...
    const filtered = designers.filter((designer) => {
      const nameMatch = nameFilter
        ? designer.name?.toLowerCase().includes(nameFilter.toLowerCase())
        : true;
      const cityMatch = cityFilter
        ? designer.city?.toLowerCase().includes(cityFilter.toLowerCase())
        : true;
      const brandMatch = brandFilter
        ? designer.brand?.toLowerCase().includes(brandFilter.toLowerCase())
        : true;
      const serviceMatch = serviceFilter
        ? designer.services?.some((s) =>
      ...
      const hoursMatch = workingHoursFilter
        ...
        : true;
      const fareMatch =
        designer.averageFare >= fareFilter[0] &&
        designer.averageFare <= fareFilter[1];
      const ratingMatch = designer.ratings?.some(
        (r) => r.rating >= ratingFilter[0] && r.rating <= ratingFilter[1]
      );
      return (
        nameMatch &&
        cityMatch &&
  ...
  return (
    <div style={styles.container}>
      <div style={styles.sidebar}>
        <h2 style={{ marginBottom: '15px' }}>Filter Designers</h2>
        <input
          type="text"
          ...
          {filteredDesigners.map(
            (designer) =>
              ...
              designer.lng && (
                <Marker
                  key={designer._id}
                  position={[designer.lat, designer.lng]}
                  icon={customIcon}
                >
                  <Popup>
                    <div style={{ maxWidth: '200px' }}>
                      <h3 style={{ margin: '5px 0' }}>{designer.name}</h3>
                      <p style={{ margin: 0 }}>{designer.city}</p>
                      <p style={{ margin: 0, wordWrap: 'break-word' }}>
                        {designer.address}
                      </p>
                      <p style={{ margin: '5px 0' }}>
                        <a href={`tel:${designer.phone}`}>📞 Call</a>
                      </p>
                      <p style={{ color: 'green', fontWeight: 'bold' }}>
                        {getAvailabilityStatus()}
                      </p>
                      <Link to={`/designer/${designer._id}`} style={styles.link}>
                        View Profile
                      </Link>
                    </div>
                  </Popup>
      ...
      <div style={styles.dataPart}>
        <ul style={styles.list}>
          {filteredDesigners.map((designer) => (
            <li key={designer._id} style={styles.listItem}>
              <div style={styles.profileImageContainer}>
                <img
                  src={
                    designer.profilePicture.startsWith('/images/')
